...
(defun max-vector (v)
  (declare (optimize (speed 3) (debug 0) (safety 0))
                  #-sbcl (type dvec v))
  #-sbcl (loop for var of-type double-float across v maximize var of-type double-float)
  #+sbcl (let ((acc (elt v 0)))
           (declare (integer acc))
           (dotimes (i (length v))
             (when (> (elt v i) acc)
               (setf acc (elt v i)))
...
(defun min-vector (v)
  (declare (optimize (speed 3) (debug 0) (safety 0))
           #-sbcl (type dvec v))
  #-sbcl (loop for var of-type double-float across v minimize var of-type double-float)
  #+sbcl (let ((acc (elt v 0)))
           (declare (integer acc))
           (dotimes (i (length v))
             (when (< (elt v i) acc)
