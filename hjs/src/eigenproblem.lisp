...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(in-package :clml.hjs.eigensystems)
;; functions:
...
(defun tred2 (a &key (eigenvectors t))
  ...
  (prog* ((n (array-dimension a 0))
     ...
     #-ccl (declare (type dvec d))
     #-ccl (declare (type dvec e))
     #-ccl (declare (type fixnum n l))
     #-ccl (declare (type (double-float 0.0 *) h))
     #-ccl (declare (type double-float scale f g hh))
     #-ccl (declare (dynamic-extent n l h scale f g hh))
...
(defun tqli (d e z &key (eigenvectors t))
  ...
  #-ccl (declare (type dvec d))
  #-ccl (declare (type symbol eigenvectors))
  #-ccl (declare (type dvec e))
  #-ccl (declare (type dmat z))

  ;; (print 'tqli)
  (prog* ((n 0) (m 0) (iter 0) (f 0d0) (s 0d0) (c 0d0)
         (b 0d0) (r 0d0) (g 0d0) (dd 0d0) (p 0d0)
         (itermax 30))
     #-ccl (declare (type fixnum n m iter itermax))
     #-ccl (declare (type double-float f s c b r g dd p))
     #-ccl (declare (dynamic-extent n m iter f s c b r g dd p))

     (setq n (array-dimension d 0))
     (do ((i 2 (+ i 1)))
         ((> i n) t)
       #-ccl (declare (type fixnum i))
       #-ccl (declare (dynamic-extent i))
       (setf (fref1 e (+ i (- 1))) (fref1 e i)))
     (setf (fref1 e n) 0d0)
     (do ((l 1 (+ l 1)))
         ((> l n) t)
       #-ccl (declare (type fixnum l))
       #-ccl (declare (dynamic-extent l))
       (setf iter 0)
      label1
       (do ((mm l (+ mm 1)))
           ((> mm (+ n (- 1))) t)
         #-ccl (declare (type fixnum mm))
         #-ccl (declare (dynamic-extent mm))
      ...
      label2
       (when (not (= m l))
         ...
         (setf r (sqrt #-ccl (the (double-float 0.0 *)
                              (+ (* g g) 1d0))
                   #+ccl(+ (* g g) 1d0)))
         ...
         (do ((i (+ m (- 1)) (+ i (- 1))))
           ...
           (cond
             ((>= (abs f) (abs g))
              (setf c (/ g f))
              (setf r (sqrt #-ccl (the (double-float 0.0 *) (+ (* c c) 1d0))
                        #+ccl (+ (* c c) 1d0)))
              (setf (fref1 e (+ i 1)) (* f r))
              (setf s (/ 1 r)) (setf c (* c s)) )
             (t
              (setf s (/ f g))
              (setf r (sqrt #-ccl (the #-ccl (double-float 0.0 *) (+ (* s s) 1d0))
                        #+ccl (+ (* s s) 1d0)))
           ...
           (when eigenvectors
             (do ((k 1 (+ k 1)))
                 ((> k n) t)
               #-ccl (declare (type fixnum k))
               #-ccl (declare (dynamic-extent k))
