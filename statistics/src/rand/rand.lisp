...
(eval-when (:compile-toplevel :load-toplevel) ; DNC
  (let* ((b 4.766d0)
         (d (/ 1d0 (* 2d0 b)))
         (a (sqrt (- (/ (* 2d0 b) pi) 1d0)))
         (s (/ a (- b a)))
...
(eval-when (:compile-toplevel) ; DNC
    (let* ((k +zuggurat-k+)
         (n (expt 2 k))
         (n-minus-1 (- n 1))
         (r (ecase k
...
(defun-with-cached-values gamma-compression-shape-big (shape)
        (declare (optimize (speed 3) (safety 0) (debug 0))
                 #+lispworks (type double-float shape)
                 #+sbcl (double-float shape)
                 #+sbcl (ignorable shape))
        ((d (- shape #.(dfloat 1/3)))
         (c (/ (sqrt (* 9d0 d)))))
        (declare #-sbcl (ignorable shape)
                 (type double-float d c))
        (loop
           (let ((z 0d0)
                 (v 0d0))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (defun-with-cached-values right-triangular-inverse (a b)
    (declare (optimize (speed 3) (safety 0) (debug 0))
             #+lispworks (type double-float a b)
             #+sbcl (double-float a b)
             #+sbcl (ignorable a b)
             )
    ((s (- b a)))
...
(defun-with-cached-values power-function-inverse (shape lower-boundary upper-boundary)
    (declare (optimize (speed 3) (safety 0) (debug 0))
             #+lispworks (type double-float shape lower-boundary upper-boundary)
             #+sbcl (double-float shape lower-boundary upper-boundary)
             #+sbcl (ignorable shape lower-boundary upper-boundary))
    ((shape-inv (/ shape))
     (s (- upper-boundary lower-boundary)))
    (declare (type double-float shape-inv s)
             #-sbcl (ignorable upper-boundary shape))
...
(declaim (ftype (function (fixnum) (double-float)) chi-square-randomn)
         (inline chi-square-random))
...
(declaim (ftype (function (fixnum) (double-float)) t-with-gamma)
           (inline t-gamma))
...
(eval-when (:compile-toplevel :load-toplevel)
  ...
  (declaim (ftype (function (fixnum) (double-float)) t-month-python)
           ;(inline t-monty-python)
           )
  ...
  (let ((u1 (* 2d0 (unit-random :[])))
    ...
    (block outer
      ...
      (let ((uy (* d (unit-random :[]))))
        #-sbcl (declare (type double-float uy))
        (when (< (* (half-integer-power (+ 1d0 (* p ux ux)) q) uy) c)
          (return-from outer))
        (let ((dash (* s (- b ux))))
          #-sbcl (declare (type double-float dash))
          ...
          (loop
            (let ((u3 (unit-random)))
              #-sbcl (declare (type double-float u3))
              (setf ux (sqrt (- (* v1 (the double-float (expt u3 (- v2)))) r)))
              (unless (>= (* ux (unit-random)) b)
                (return-from outer)))))))
 `    (* sign ux)))

  (declaim (ftype (function (fixnum) (double-float)) t-month-python-bit))
    ...
    ((r (dfloat freedom))
     (b (case freedom
          ...
          (t
           ...
           (+ 2.5074d0 (expt (* 1.876d0 r) -1.042d0)))))
     (c (multiple-value-bind (quotient remainder) (floor freedom 2)
          #-sbcl (declare (type fixnum quotient remainder))
          (if (= remainder 0)
            ;;; even
              (/ (loop with ans = 1d0
                    for i fixnum from 1 below quotient do
     ...
     (v2 (/ 2d0 (- r 1d0))))
    (declare
     #+lispworks (type double-float r b c a d s q t1 t2 v1 v2)
     #+lispworks (type rational q)
...
#+ignore
(declare
 (type double-float r b c a d w s p t1 t2 v1 v2)
           ;(type rational q)
           (type fixnum k)
           (ignorable freedom a))
...
(defun-with-cached-values binomial-inverse-mode (size probability)
    (declare (optimize (speed 3) (safety 0) (debug 0))
             ...
             #-sbcl (type fixnum size)
             #+sbcl (fixnum size)
             #+sbcl (double-float probability)
             #+sbcl (ignorable size probability))
...
(defun-with-cached-values geometric-inverse-exp (probability)
  ...
  (declare #-sbcl (ignorable probability)
           (type double-float d))
...
(eval-when (:compile-toplevel :load-toplevel)
  (defun-with-cached-values poisson-simulate (rate)
    (declare (optimize (speed 3) (safety 0) (debug 0))
             #-sbcl (type double-float rate)
             #+sbcl (double-float rate)
             #+sbcl (ignorable rate))
    ((d (exp rate)))
...
(defun-with-cached-values hypergeometric-inverse (elements successes samples)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           #-sbcl (type fixnum elements successes samples)
           #+sbcl (fixnum elements successes samples)
           #+sbcl (ignorable elements successes samples)
           )
...
(eval-when (:compile-toplevel :load-toplevel)
  (defun-with-cached-values negative-binomial-compose (successes probability)
    (declare (optimize (speed 3) (safety 0) (debug 0))
             #-sbcl (type double-float successes probability)
             #+sbcl (double-float successes probability)
             #+sbcl (ignorable probability successes))
...
(eval-when (:load-toplevel)
  (defun-with-cached-values negative-binomial-inverse (successes probability)
    ...
    (declare (type double-float d q s)
             #-sbcl (ignorable successes probability))
    (let ((x 0)
          (p d)
          (u (unit-random :[)))
      (declare (type fixnum x)
